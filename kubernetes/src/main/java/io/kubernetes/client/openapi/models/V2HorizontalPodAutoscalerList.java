...
public class V2HorizontalPodAutoscalerList implements io.kubernetes.client.common.KubernetesListObject {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V2HorizontalPodAutoscalerList
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V2HorizontalPodAutoscalerList.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V2HorizontalPodAutoscalerList.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V2HorizontalPodAutoscalerList` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayitems.size(); i++) {
        V2HorizontalPodAutoscaler.validateJsonObject(jsonArrayitems.get(i).getAsJsonObject());
      };
