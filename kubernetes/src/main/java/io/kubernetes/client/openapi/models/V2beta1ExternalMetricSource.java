...
public class V2beta1ExternalMetricSource {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V2beta1ExternalMetricSource
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V2beta1ExternalMetricSource.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in V2beta1ExternalMetricSource is not found in the empty JSON string", V2beta1ExternalMetricSource.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V2beta1ExternalMetricSource.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V2beta1ExternalMetricSource` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : V2beta1ExternalMetricSource.openapiRequiredFields) {
      ...
      if (jsonObj.get("metricSelector") != null && !jsonObj.get("metricSelector").isJsonNull()) {
        V1LabelSelector.validateJsonObject(jsonObj.getAsJsonObject("metricSelector"));
      }
      ...
      if ((jsonObj.get("targetValue") != null && !jsonObj.get("targetValue").isJsonNull()) && !jsonObj.get("targetValue").isJsonPrimitive()) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!V2beta1ExternalMetricSource.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'V2beta1ExternalMetricSource' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<V2beta1ExternalMetricSource> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(V2beta1ExternalMetricSource.class));

       return (TypeAdapter<T>) new TypeAdapter<V2beta1ExternalMetricSource>() {
           @Override
           public void write(JsonWriter out, V2beta1ExternalMetricSource value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public V2beta1ExternalMetricSource read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of V2beta1ExternalMetricSource given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of V2beta1ExternalMetricSource
  * @throws IOException if the JSON string is invalid with respect to V2beta1ExternalMetricSource
  */
  public static V2beta1ExternalMetricSource fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, V2beta1ExternalMetricSource.class);
  }

 /**
  * Convert an instance of V2beta1ExternalMetricSource to an JSON string
  *
  * @return JSON string
  */
