...
public class V1ControllerRevision implements io.kubernetes.client.common.KubernetesObject {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V1ControllerRevision
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V1ControllerRevision.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V1ControllerRevision.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1ControllerRevision` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
