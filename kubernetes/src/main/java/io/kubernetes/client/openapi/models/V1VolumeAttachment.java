...
public class V1VolumeAttachment implements io.kubernetes.client.common.KubernetesObject {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V1VolumeAttachment
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V1VolumeAttachment.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in V1VolumeAttachment is not found in the empty JSON string", V1VolumeAttachment.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V1VolumeAttachment.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1VolumeAttachment` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : V1VolumeAttachment.openapiRequiredFields) {
      ...
      V1VolumeAttachmentSpec.validateJsonObject(jsonObj.getAsJsonObject("spec"));
      // validate the optional field `status`
      if (jsonObj.get("status") != null && !jsonObj.get("status").isJsonNull()) {
        V1VolumeAttachmentStatus.validateJsonObject(jsonObj.getAsJsonObject("status"));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!V1VolumeAttachment.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'V1VolumeAttachment' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<V1VolumeAttachment> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(V1VolumeAttachment.class));

       return (TypeAdapter<T>) new TypeAdapter<V1VolumeAttachment>() {
           @Override
           public void write(JsonWriter out, V1VolumeAttachment value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public V1VolumeAttachment read(JsonReader in) throws IOException {
  ...
  * @return An instance of V1VolumeAttachment
  * @throws IOException if the JSON string is invalid with respect to V1VolumeAttachment
  */
  public static V1VolumeAttachment fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, V1VolumeAttachment.class);
  }
