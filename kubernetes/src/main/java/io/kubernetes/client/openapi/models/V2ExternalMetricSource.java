...
public class V2ExternalMetricSource {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V2ExternalMetricSource
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V2ExternalMetricSource.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V2ExternalMetricSource.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V2ExternalMetricSource` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
