...
public class V1HTTPHeader {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V1HTTPHeader
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V1HTTPHeader.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V1HTTPHeader.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1HTTPHeader` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
