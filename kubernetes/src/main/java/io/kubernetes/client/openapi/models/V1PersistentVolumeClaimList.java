...
public class V1PersistentVolumeClaimList implements io.kubernetes.client.common.KubernetesListObject {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V1PersistentVolumeClaimList
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V1PersistentVolumeClaimList.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V1PersistentVolumeClaimList.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1PersistentVolumeClaimList` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayitems.size(); i++) {
        V1PersistentVolumeClaim.validateJsonObject(jsonArrayitems.get(i).getAsJsonObject());
      };
