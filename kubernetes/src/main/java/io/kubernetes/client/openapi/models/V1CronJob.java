...
public class V1CronJob implements io.kubernetes.client.common.KubernetesObject {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V1CronJob
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V1CronJob.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V1CronJob.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1CronJob` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("spec") != null && !jsonObj.get("spec").isJsonNull()) {
        V1CronJobSpec.validateJsonObject(jsonObj.getAsJsonObject("spec"));
      }
      // validate the optional field `status`
      if (jsonObj.get("status") != null && !jsonObj.get("status").isJsonNull()) {
        V1CronJobStatus.validateJsonObject(jsonObj.getAsJsonObject("status"));
      }
  }
