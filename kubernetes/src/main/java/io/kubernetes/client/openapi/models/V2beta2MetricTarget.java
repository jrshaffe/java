...
public class V2beta2MetricTarget {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V2beta2MetricTarget
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V2beta2MetricTarget.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in V2beta2MetricTarget is not found in the empty JSON string", V2beta2MetricTarget.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V2beta2MetricTarget.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V2beta2MetricTarget` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : V2beta2MetricTarget.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!V2beta2MetricTarget.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'V2beta2MetricTarget' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<V2beta2MetricTarget> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(V2beta2MetricTarget.class));

       return (TypeAdapter<T>) new TypeAdapter<V2beta2MetricTarget>() {
           @Override
           public void write(JsonWriter out, V2beta2MetricTarget value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public V2beta2MetricTarget read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of V2beta2MetricTarget given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of V2beta2MetricTarget
  * @throws IOException if the JSON string is invalid with respect to V2beta2MetricTarget
  */
  public static V2beta2MetricTarget fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, V2beta2MetricTarget.class);
  }

 /**
  * Convert an instance of V2beta2MetricTarget to an JSON string
  *
  * @return JSON string
  */
