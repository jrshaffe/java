...
import io.kubernetes.client.openapi.models.V2beta2ContainerResourceMetricStatus;
...
public class V2beta2MetricStatus {
  ...
  public V2beta2MetricStatus containerResource(V2beta2ContainerResourceMetricStatus containerResource) {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V2beta2MetricStatus
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V2beta2MetricStatus.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in V2beta2MetricStatus is not found in the empty JSON string", V2beta2MetricStatus.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V2beta2MetricStatus.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V2beta2MetricStatus` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : V2beta2MetricStatus.openapiRequiredFields) {
      ...
      if (jsonObj.get("containerResource") != null && !jsonObj.get("containerResource").isJsonNull()) {
        V2beta2ContainerResourceMetricStatus.validateJsonObject(jsonObj.getAsJsonObject("containerResource"));
      }
      // validate the optional field `external`
      if (jsonObj.get("external") != null && !jsonObj.get("external").isJsonNull()) {
        V2beta2ExternalMetricStatus.validateJsonObject(jsonObj.getAsJsonObject("external"));
      }
      // validate the optional field `object`
      if (jsonObj.get("object") != null && !jsonObj.get("object").isJsonNull()) {
        V2beta2ObjectMetricStatus.validateJsonObject(jsonObj.getAsJsonObject("object"));
      }
      // validate the optional field `pods`
      if (jsonObj.get("pods") != null && !jsonObj.get("pods").isJsonNull()) {
        V2beta2PodsMetricStatus.validateJsonObject(jsonObj.getAsJsonObject("pods"));
      }
      // validate the optional field `resource`
      if (jsonObj.get("resource") != null && !jsonObj.get("resource").isJsonNull()) {
        V2beta2ResourceMetricStatus.validateJsonObject(jsonObj.getAsJsonObject("resource"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!V2beta2MetricStatus.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'V2beta2MetricStatus' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<V2beta2MetricStatus> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(V2beta2MetricStatus.class));

       return (TypeAdapter<T>) new TypeAdapter<V2beta2MetricStatus>() {
           @Override
           public void write(JsonWriter out, V2beta2MetricStatus value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public V2beta2MetricStatus read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of V2beta2MetricStatus given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of V2beta2MetricStatus
  * @throws IOException if the JSON string is invalid with respect to V2beta2MetricStatus
  */
  public static V2beta2MetricStatus fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, V2beta2MetricStatus.class);
  }

 /**
  * Convert an instance of V2beta2MetricStatus to an JSON string
  *
  * @return JSON string
  */
