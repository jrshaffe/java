...
public class V1Preconditions {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V1Preconditions
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V1Preconditions.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in V1Preconditions is not found in the empty JSON string", V1Preconditions.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V1Preconditions.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1Preconditions` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!V1Preconditions.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'V1Preconditions' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<V1Preconditions> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(V1Preconditions.class));

       return (TypeAdapter<T>) new TypeAdapter<V1Preconditions>() {
           @Override
           public void write(JsonWriter out, V1Preconditions value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public V1Preconditions read(JsonReader in) throws IOException {
